...
def logged_driver(driver: WebDriver, configs: Config) -> WebDriver:
    if AUTH_STATE_COOKIES.exists() and not expires_in_x_seconds(AUTH_STATE_COOKIES):
...
def free_logged_driver(driver: WebDriver, configs: Config) -> WebDriver:
    ...
    if FREE_PLAN_STATE_COOKIES.exists() and not expires_in_x_seconds(FREE_PLAN_STATE_COOKIES):
        driver.get(configs.base_app_url)
    ...
    projects_page.choose_company_in_dropdown_list(projects_page.company_options.free_projects)
