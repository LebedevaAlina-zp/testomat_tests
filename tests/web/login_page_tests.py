...
def test_login_invalid(app: Application, invalid_email, invalid_password):
    (app.login_page.open().is_loaded().login(invalid_email, invalid_password).is_invalid_login_message_visible())


@pytest.mark.smoke
@pytest.mark.web
def test_login_valid(app: Application, configs: Config):
    (app.home_page.open().is_loaded().click_login_button())

    (app.login_page.is_loaded().login(configs.email, configs.password, remember_me=True))

    (app.projects_page.is_sign_in_successful_message_visible().is_loaded())
